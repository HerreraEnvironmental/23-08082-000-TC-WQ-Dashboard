...
server <- function(input, output, session) {
      ...
      write.csv(
        trend_summary() %>%
          select(
            ...
            EndYear,
            Season,
            CorrectedForAutocorrelation,
            `p-value` = p,
            `Slope (units/years)` = Slope
          ),
        con,
        row.names = F
      )
  ...
  output$wqi_annual <- renderPlotly({
    ggplotly(wqi_annual_plot(annual_wqi, input), source = "wqi_year_select") %>%
  ...
  wqi_data_present <- reactive({
    ...
    input$wqi_year %in%
      data_missing_test$WaterYear &
      input$main_site %in% data_missing_test$site == TRUE
  })
  ...
  output$data_missing_message <- renderUI({
    if (wqi_data_present()) {
      ...
      h4(
        "There is not enough data to display the monthly WQI. Please adjust your selection to display the data."
      )
    }
  })
  ...
  observeEvent(event_data("plotly_click", source = "wqi_year_select"), {
    ...
    updateSelectInput(session, "wqi_sum_year", selected = s)
    updateSelectInput(session, "wqi_year", selected = s)
  })
  ...
  observeEvent(input$map_marker_click, {
    ...
    updateVirtualSelect(session = session, "main_site", selected = click$id)
    updateVirtualSelect(session = session, "main_site2", selected = click$id)
    updateVirtualSelect(session = session, "main_site3", selected = click$id)
    updateVirtualSelect(session = session, "main_site4", selected = click$id)
    updateVirtualSelect(session = session, "main_site5", selected = click$id)
  })
