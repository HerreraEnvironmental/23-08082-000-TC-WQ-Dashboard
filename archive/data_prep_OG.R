...
streams_sites <- read.csv(
  'inputs/Herrera All Stream Data Dump 4 12 2023.csv'
) %>%
...
## Original
streams_wq_dat <- read.csv(
  'inputs/Herrera All Stream Data Dump 4 12 2023.csv'
) %>%
  tibble() %>%
  mutate(
    DateTime = with_tz(
      as_datetime(date_time, format = '%m/%d/%Y %H:%M', tz = 'UTC') +
        hours(sample_utc_offset),
...
streams_wq_dat["parameter"][
  streams_wq_dat["parameter"] == "Temperature, water"
] <- "Water Temperature (°C)"
