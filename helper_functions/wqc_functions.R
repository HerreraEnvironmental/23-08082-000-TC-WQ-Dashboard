...
wqc_finder <- function(
  ...
  if (
    Parameter %in%
      c('Temperature', 'Temperature, water', 'Water Temperature (°C)')
  ) {
  ...
  if (Parameter %in% c('E. coli', 'Fecal Coliform')) {
    wqc_out <- tibble(Month = Month, Result = Result) |>
      mutate(
        Season = factor(
          ifelse(
            Month <= 3,
            'Winter',
            ifelse(Month <= 6, 'Spring', ifelse(Month <= 9, 'Summer', 'Fall'))
          ),
      ...
      summarise(
        ...
        p90 = ifelse(n() <= 10, max(Result), quantile(Result, p = .9)),
        .groups = 'drop'
      ) |>
      mutate(
        Season,
