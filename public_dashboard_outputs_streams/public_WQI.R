...
stream_use_designations <- read_xlsx(
...
wqi_calc <- function(
  ...
  if (any(shortParmName == 'Temp' & missing(TemperatureCode))) {
    stop(
      'Must provide temperature code (depends on WQ Standards) to calculate WQI for temperature'
    )
  }
  if (any(shortParmName == 'Oxygen' & missing(OxygenCode))) {
    stop(
      'Must provide oxygen code (depened on WQ Standards) to calculate WQI for oxygen'
    )
  }
  if (missing(site)) {
    site = 'A'
    warning(
      'No site name provided. Assuming data are all from same site ("A").'
    )
  }
  if (
    !(any(
      shortParmName %in%
        c('FC', 'Oxygen', 'pH', 'TP_P', 'SUSSOL', 'Temp', 'TPN', 'Turb')
    ))
  ) {
    stop(
      'Must provide parameter name in shorterned form:\n
                FC - Fecal coliform\n
  ...
  wqi_prep <- tibble(
    ...
    mutate(
      rownumber = ifelse(
        ...
        ifelse(
          shortParmName == 'TP_P',
          ifelse(small_PS_stream, 72, 272),
          ifelse(
            shortParmName == 'Oxygen',
            OxygenCode,
            ifelse(
              shortParmName == 'TPN',
              ifelse(small_PS_stream, 62, 262),
              ifelse(
                shortParmName == 'Turb',
                92,
                ifelse(
                  shortParmName == 'SUSSOL',
                  82,
                  ifelse(
                    shortParmName == 'FC',
                    51,
  ...
  if (any(shortParmName %in% c('SUSSOL', 'Turb'))) {
    sediment_score <- wqi_prep %>%
      ...
      summarise(shortParmName = 'Sediment', WQI = sum(WQI^-1)^-1 * length(WQI))
  } else {
    sediment_score <- NULL
  }
