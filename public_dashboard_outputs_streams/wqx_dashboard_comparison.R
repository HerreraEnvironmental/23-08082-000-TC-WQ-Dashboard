...
diffs <- t_wqp |>
  ...
  filter(
    is.na(n_wqp)        | is.na(n_streams)        | n_wqp        != n_streams |
      is.na(startdate_wqp) | is.na(startdate_streams) | startdate_wqp != startdate_streams |
      is.na(enddate_wqp)   | is.na(enddate_streams)   | enddate_wqp   != enddate_streams
  ) |>
  arrange(parameter)
